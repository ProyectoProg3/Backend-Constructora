...
export class CargarArchivoController {
   ...
   @post('/CargarDocumentoCliente', {
    responses: {
      200: {
        ...
        description: 'Función de carga del documento del cliente',
      },
    },
  })
  async cargarDocumentoCliente(
    @inject(RestBindings.Http.RESPONSE) response: Response,
    @requestBody.file() request: Request,
  ): Promise<object | false> {
    const rutaDocumentoCliente = path.join(__dirname, llaves.carpetaDocumentosClientes);
    let res = await this.StoreFileToPath(rutaDocumentoCliente, llaves.nombreCampoDocumentoCliente, request, response, llaves.extensionesPermitidasDOC);
    if (res) {
      const nombre_archivo = response.req?.file.filename;
      if (nombre_archivo) {
        return {filename: nombre_archivo};
  ...
  /**
   * Retorna una configuración para el almacenamiento multer
   * @param path
   */
    ...
    return new Promise<object>((resolve, reject) => {
      ...
      const upload = multer({
        ...
        fileFilter: function (req: any, file: any, callback: any) {
          ...
          return callback(new HttpErrors[400]('El formato del archivo no está permitido.'));
        },
        limits: {
