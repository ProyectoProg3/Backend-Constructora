...
export class CargarArchivoController {
  ...
  async cargarDocumentoCliente(
    ...
    const rutaDocumentoCliente = path.join(__dirname, llaves.carpetaImagenesProyectos);
    let res = await this.StoreFileToPath(rutaDocumentoCliente, llaves.nombreCampoDocumentoCliente, request, response, llaves.extensionesPermitidasIMG);
    if (res) {
      const nombre_archivo = response.req?.file.filename;
      if (nombre_archivo) {
        return {filename: nombre_archivo};
